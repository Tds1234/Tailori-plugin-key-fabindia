...
(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		_createUrl: function (RenderObject,IsBlocking,onlycall) {
			...
			if (this._Url.indexOf("p=") === -1) {
				...
				if(onlycall != undefined){
					url.replace("ALL",this._SelectedAlignment);
					return url;
				}else{
					$.ajax({
					url: url,
					...
					success: function (data) {
						...
						this._ImageUrl.replace("ALL",this._SelectedAlignment)
		...
		_placeImages : function(imgSrc,data,c){
			...
			if (!isAny) {
				...
				$(imgSrc + " img:last").attr("data-zoom-image", this.Option("ServiceUrl") + "/v1/img?key="+this.Option("Key") + "&"+ this._Url.replace("ALL",this._SelectedAlignment) + "/type=5");
				...
				$(imgSrc + ' .TdsNew').on('load', function() {
					...
					if(loadedImage == $(imgSrc + ' .TdsNew').length){
						...
						var callback = that.Option("OnRenderImageChange");
						if (typeof callback == 'function')
						callback.call(that, that._Images);
					}
				});
				/*.each(function() {
			...
			if (rawRenderData === undefined) {
				...
				/*$.ajax({
					...
					beforeSend: function (xhr) {
						...
						if(beforeSendfn !== undefined && typeof beforeSendfn == 'function'){
							beforeSendfn();
						}
					},
	...
	function capitalizeFLetter(str) { 
		var string = str.trim().split(" "),result="";
